...
event face_field (i++)
{
  ...
  for (scalar M in surface)
    if (M.alpha) {
      foreach() {
        if (M.M0) {
          sig[] -= M.alpha * (M[]/area(point) - M.M0);
        }
        else{
          double theta = M[]/(area(point) * M.max);
          sig[] += M.alpha * M.max * (log(1 - theta) - M.A/2 * pow(theta,2));
        } 
      }
    }
  boundary({sig});
...
scalar adsorption_term (scalar M)
{
  ...
  foreach() {
    if (M.M0) {
      ads[] = M.omega * ((M.M0 + M.K * (c[0,0,nl-1] - M.c0)) * area(point) - M[]);
    }
    else {
      double theta = M[]/(area(point) * (M.max ? M.max : HUGE));
      ads[] = M.omega * (M.K * c[0,0,nl-1] * area(point) * (1 - theta) - M[] * exp(- M.A * theta));
    }
  }
  boundary({ads});
  return ads;
...
event face_fields (i++) 
{
  for (scalar M in surface)
    if (M.omega && M.K) {
      double dt_max = 1/(10.*M.omega*M.K);
...
/**
The function is called before the remap step (so as to have the new surface concentration for next timestep).
Improvements : implicit this step to remove the strong stability condition */
event remap  (i++)
{
  for (scalar M in surface)
    if (M.omega && M.K) {
