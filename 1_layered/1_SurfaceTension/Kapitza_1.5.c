...
![Surface Profil](Kapitza_1.5/movie.mp4)
![Surface Profil of Malamataris](Kapitza/Malamataris1.5.png)
*/

#define F 1.5 //dimensionned frequency (Hz)
#define k (2.*pi*F*T0)

#define T_END (0.6*1.8/H0)
...
void plot (FILE * fp)
{
  ...
  foreach (serial) {
    ...
    foreach_layer() {
      z += h[];
      fprintf (fp, "%g %g %g\n", x*H0*1000, min(max(0.5, z), 1.8), u.x[]);
    }
    fprintf (fp, "\n");
  }
  fprintf (fp, "e\n\n");
